...
try:
    ...
    import nltk
    # Only need to download punkt once
    try:
...
def summarise_articles(articles: list[Article]) -> list[Article]:
    """In-place summary of article text."""
    for art in articles:
        if art.body:
            art.summary = summarise(art.body)
            art.takeaway = extract_takeaway(art.body, art.title)
